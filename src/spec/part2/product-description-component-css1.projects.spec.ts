...
describe('ProductDescriptionComponent', () => {
  ...
  it(`should have CSS that contains a paragraph selector @product-description-component-css1`, async(() => {
    ...
    if (productDescriptionCssFileExists) {
      ...
      since('There isn\'t a paragraph selector in the ProductDescriptionComponent\'s CSS file right now.')
      .expect(_.find(parsed.cssRules, {selectorText: 'p'})).not.toBeUndefined();
    }
  }));

  it(`should have CSS with a rule setting the font-size to 16px on the paragraph
  selector @product-description-component-css1`, async(() => {
    since('The ProductDescriptionComponent hasn\'t been created yet.').expect(productDescriptionCssFileExists).toBe(true);
    if (productDescriptionCssFileExists) {
      const parsed = CSSOM.parse(productDescriptionCssFile);
      ...
      since('Your paragraph selector doesn\'t have a `font-size` property that\'s equal to `16px`.')
      .expect(pRule.style['font-size']).toBe('16px');
    }
  }));

  it(`should have CSS with a rule setting the font-family to Helvetica, Arial, sans-serif on the paragraph
  selector @product-description-component-css1`, async(() => {
    since('The ProductDescriptionComponent hasn\'t been created yet.').expect(productDescriptionCssFileExists).toBe(true);
    if (productDescriptionCssFileExists) {
      const parsed = CSSOM.parse(productDescriptionCssFile);
      ...
      if (fontRule !== undefined) {
        ...
        since('Your paragraph selector doesn\'t have a `font-family` property that\'s equal to `Helvetica, Arial, sans-serif`.')
        .expect(split[0]).toBe('Helvetica');
        since('Your paragraph selector doesn\'t have a `font-family` property that\'s equal to `Helvetica, Arial, sans-serif`.')
        .expect(split[1]).toBe('Arial');
        since('Your paragraph selector doesn\'t have a `font-family` property that\'s equal to `Helvetica, Arial, sans-serif`.')
        .expect(split[2]).toBe('sans-serif');
      }
    }
  }));

  it(`should have CSS with a rule setting the font-weight to normal on the paragraph
  selector @product-description-component-css1`, async(() => {
    since('The ProductDescriptionComponent hasn\'t been created yet.').expect(productDescriptionCssFileExists).toBe(true);
    if (productDescriptionCssFileExists) {
      const parsed = CSSOM.parse(productDescriptionCssFile);
      ...
      since('There isn\'t a paragraph selector in the ProductDescriptionComponent\'s CSS file right now.')
      .expect(pRule.style.parentRule.selectorText).toBe('p');
      since('Your paragraph selector doesn\'t have a `font-weight` property that\'s equal to `normal`.')
      .expect(pRule.style['font-weight']).toBe('normal');
    }
  }));
